...
def main(workflow):
    try:
        ...
        api_key = workflow.get_password('gitlab_api_key')
        api_url = workflow.settings.get('api_url')
        ...
        projects = workflow.cached_data('projects', wrapper, max_age=3600)
        ...
        workflow.logger.error('No API key saved')
